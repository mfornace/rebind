...
#define ARA_TMP(Q) template <class R, class C, class ...Ts> \
...
std::false_type third_is_convertible(Pack<R, S>);

template <class C, class R, class S, class T, class ...Ts>
std::is_convertible<T, C> third_is_convertible(Pack<R, S, T, Ts...>);
...
constexpr auto simplify_return_type(Type<T>) {
    using U = std::remove_reference_t<T>;
    static_assert(!std::is_volatile_v<U> || !std::is_reference_v<T>, "volatile references are not supported");
    using Out = std::conditional_t<std::is_lvalue_reference_v<T>, U &, std::remove_cv_t<U>>;
    static_assert(std::is_same_v<Out, T> || std::is_convertible_v<T, Out>, "simplified type should be compatible with original");
    return Type<Out>();
}

inline constexpr Type<void> simplify_return_type(Type<void> t) {return t;}

template <class R, class ...Ts>
Pack<decltype(*simplify_return_type(Type<R>())), decltype(*simplify_argument(Type<Ts>()))...> simplify_signature_f(Pack<R, Ts...>) {return {};}
