...
#define SFB_DETECT_TRAIT(NAME, SIGNATURE) \
    template <class T, class SFINAE=void> \
...
struct Attribute {
    ...
    template <class T>
    struct Default {
        static stat attribute_nothrow(Target& out, Pointer self, sfb_str name, Mode mode) noexcept {
    ...
    static stat invoke(Idx f, Target &out, Pointer self, sfb_str name, Mode qualifier) noexcept {
...
SFB_DETECT_TRAIT(has_operator_equal, decltype(true || std::declval<T const&>() == std::declval<T const&>()));
SFB_DETECT_TRAIT(has_equal, decltype(Impl<T>::equal(std::declval<T const&>(), std::declval<T const&>(), DeclAny())));
...
SFB_DETECT_TRAIT(has_compare, decltype(Impl<T>::compare(std::declval<T const&>(), std::declval<T const&>(), DeclAny())));
SFB_DETECT_TRAIT(has_operator_less, decltype(true || std::declval<T const&>() < std::declval<T const&>()));
...
SFB_DETECT_TRAIT(has_hash, decltype(std::size_t() + std::hash<T>()(std::declval<T const&>())));
...
SFB_DETECT_TRAIT(has_dump, decltype(Impl<T>::dump(std::declval<Target&>(), std::declval<T&&>())));
...
SFB_DETECT_TRAIT(has_load, decltype(Impl<T>::load(std::declval<Ref&>())));
    ...
    struct Default {
        ...
        static stat load_nothrow(Target &out, Pointer source, Mode qualifier) noexcept {
            if constexpr(has_load_v<T>) {
                ...
                return out.make_noexcept([&] {
                    sfb_ref r{sfb_mode_index(out.index(), static_cast<sfb_mode>(qualifier)), source.base};
                    out.set_index<T>();
...
SFB_DETECT_TRAIT(has_call, decltype(Impl<T>::call(DeclAny())));
...
struct Switch {
    ...
    static Stat invoke(sfb_input i, void* o, void* s, void* args) noexcept {
...
struct Switch<void, SFINAE> {
    static Stat invoke(sfb_input i, void* o, void* s, void*) noexcept {
        switch (i.code) {
            case code::name: {
