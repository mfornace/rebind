...
template <> struct AliasType<Index>  {using type = sfb_index;};
...
struct Tagged {
    ...
    Tagged(Idx i, Mode t) : base(sfb_mode_index(i, static_cast<sfb_mode>(t))) {}
    ...
    auto tag() const noexcept {return static_cast<Mode>(sfb_get_mode(base));}
    explicit operator Index() const noexcept {return sfb_get_index(base);}
...
template <>
struct TypeName<sfb_index> {
    static constexpr std::string_view const name = "sfb::Index";
};
...
template <>
struct hash<sfb::Index> {
    size_t operator()(sfb::Index i) const {return hash<sfb::Idx>()(i.base);}
};

template <class T>
struct hash<sfb::Tagged<T>> {
    size_t operator()(sfb::Tagged<T> i) const {return hash<sfb::Idx>()(i.base);}
};
